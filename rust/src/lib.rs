...
pub fn initialize_dirs<'local>(context: JObject<'local>, external_dir_uri: JObject<'local>) {
    ...
    initialize_dirs(env, external_dir_uri, &app_dir, context).unwrap_or_else(|err| panic!("{err}"))
}
...
fn initialize_dirs<'local>(
    ...
    let external_dir = ExternalDir::new(env, context, external_dir_uri)
        .ok_or("Couldn't open external directory")?;
...
fn merge_dirs<'local>(
    ...
    let internal_files = internal_dir
        .read_dir()
    ...
    for entry in copy_to_external {
        ...
        let mut external_file = match external_dir.create_file(
            ...
            Ok(file) => file
                ...
                .map_err(|err| {
                    format!("Failed to open newly created file in external directory: {err}")
                })?,
            Err(error) => {
                return if error.kind() == ErrorKind::AlreadyExists {
...
fn import_file_external<'local>(
    ...
    let mut external_file =
        ExternalDir::new(env, env.new_local_ref(context).unwrap(), external_dir_uri)
